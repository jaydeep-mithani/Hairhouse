...
import { json, type ActionArgs, type AppLoadContext, defer, type LoaderArgs } from '@shopify/remix-oxygen'
...
import { isLocalPath, extractCookieValue } from '~/lib/utils'
...
export async function loader({ request, context }: LoaderArgs) {
  ...
  const { page, header, footer }: CMsPagesQuery = await client
    .query(CMsPagesDocument, { url: 'cart' })
    .toPromise()
    .then((result) => {
      return {
  ...
  const recsProps = { pageType: 'CART', uid: unbxdUserId, unbxdParams }
...
export default function CartRoute() {
  ...
  const { page, header, footer, unbxdRecs } = useLoaderData<typeof loader>()

  return (
    <>
      {header && <HeaderSection page={page} {...header} />}
      <div className="grid w-full gap-8 justify-items-start">
        <Suspense fallback={<CartLoading />}>
          <Await resolve={root.data?.cart}>
            {(cart) => {
              return <Cart layout="page" cart={cart} unbxdRecs={unbxdRecs} />
            }}
          </Await>
        </Suspense>
      </div>
