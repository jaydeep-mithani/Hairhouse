...
import { json, redirect, type MetaFunction, type ActionFunction } from '@shopify/remix-oxygen'
...
export const action: ActionFunction = async ({ request, context, params: { lang, id, resetToken } }) => {
  ...
  try {
    const data = await storefront.mutate<{ customerReset: CustomerResetPayload }>(CUSTOMER_RESET_MUTATION, {
      variables: {
...
export default function Reset() {
  ...
  const [nativePasswordConfirmError, setNativePasswordConfirmError] = useState<null | string>(null)
      ...
      <div className="max-w-md w-full">
        ...
        <Form method="post" noValidate className="pt-6 pb-8 mt-4 mb-4 space-y-3">
          ...
          <div className="mb-3">
            ...
            />
            {nativePasswordConfirmError && <p className="text-red-500 text-xs"> {nativePasswordConfirmError} &nbsp;</p>}
          </div>
