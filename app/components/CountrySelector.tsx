...
import { Locale, CartAction, Localizations } from '~/lib/type'
...
export function CountrySelector() {
  ...
  return (
    <section ref={observerRef} className="grid w-full gap-4 md:max-w-xs md:ml-auto" onMouseLeave={closeDropdown}>
          ...
          <div className="w-full overflow-auto border-t border-contrast/30 dark:border-white bg-contrast/30 max-h-36">
            {countries &&
              Object.keys(countries).map((countryPath) => {
                ...
                const isSelected =
                  countryLocale.language === selectedLocale.language && countryLocale.country === selectedLocale.country
...
function ChangeLocaleForm({
  ...
  return (
    <fetcher.Form action="/cart" method="post">
      <input type="hidden" name="cartAction" value={CartAction.UPDATE_BUYER_IDENTITY} />
      <input type="hidden" name="buyerIdentity" value={JSON.stringify(buyerIdentity)} />
