...
import type { Maybe, PageInfo, ProductConnection } from '@shopify/hydrogen/storefront-api-types'
...
export function Pagination<Resource extends Connection>({
  ...
  const { endCursor, hasNextPage, hasPreviousPage, nextPageUrl, nodes, prevPageUrl, startCursor } =
 ...
 */
export function usePagination(connection: Connection): Omit<PaginationInfo, 'isLoading' | 'nextLinkRef'> {
  ...
  const currentPageInfo = useMemo(() => {
    let pageStartCursor = state?.pageInfo?.startCursor === undefined ? startCursor : state.pageInfo.startCursor

    let pageEndCursor = state?.pageInfo?.endCursor === undefined ? endCursor : state.pageInfo.endCursor
    ...
    const previousPageExists =
      state?.pageInfo?.hasPreviousPage === undefined ? hasPreviousPage : state.pageInfo.hasPreviousPage

    const nextPageExists = state?.pageInfo?.hasNextPage === undefined ? hasNextPage : state.pageInfo.hasNextPage
  ...
  const prevPageUrl = useMemo(() => {
    ...
    currentPageInfo.startCursor && params.set('cursor', currentPageInfo.startCursor)
